...
%%% Final COP Position Constraint %%%
AcopEndx = dynamics.PcopU(end,:);
bcopEndx = gait.COPs{end}(1) - dynamics.PcopX(end,:)*dynamics.initialConditionX;
AcopEndy = dynamics.PcopU(end,:);
bcopEndy = gait.COPs{end}(2) - dynamics.PcopX(end,:)*dynamics.initialConditionY;
AcopEnd = blkdiag(AcopEndx,AcopEndy);
bcopEnd = [bcopEndx;bcopEndy];

[Aeq,beq] = add_constraint(Aeq,beq,AcopEnd,bcopEnd);

%%% Final DS1 COP Position Constraint %%%
AcopEndx = dynamics.PcopU(constants.Nds1,:);
bcopEndx = gait.COPs{2}(1) - dynamics.PcopX(constants.Nds1,:)*dynamics.initialConditionX;
AcopEndy = dynamics.PcopU(constants.Nds1,:);
bcopEndy = gait.COPs{2}(2) - dynamics.PcopX(constants.Nds1,:)*dynamics.initialConditionY;
AcopEnd = blkdiag(AcopEndx,AcopEndy);
bcopEnd = [bcopEndx;bcopEndy];
